...
export function UploadDocumentsForm() {
  ...
  return (
    <form onSubmit={ingest} className="flex flex-col h-3/6 w-full mb-4">
      <textarea
        className="grow mr-8 p-4 rounded w-full my-4"
        placeholder="Paste your document here"
        value={document}
        onChange={(e) => setDocument(e.target.value)}
      ></textarea>
      <button type="submit" className={`${document.length === 0 ? "cursor-not-allowed disabled:opacity-50" : ""} shrink-0 px-8 py-4 bg-sky-600 rounded w-28`} disabled={document.length === 0}>
